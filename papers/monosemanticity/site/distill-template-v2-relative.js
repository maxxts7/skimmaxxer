...
`);
  ...
  function author_string(ent, template, sep, finalSep) {
    ...
    let name_strings = names.map(name => {
      ...
      return template
        ...
        .replace(/,$/, ""); // single-word names have no initials — drop the dangling comma
    });
    if (names.length > 1) {
  ...
  function texify(str) {
    // minimal LaTeX-isms: --- is an em dash, -- is an en dash (page ranges)
  ...
  function venue_string(ent) {
    ...
    if ("volume" in ent) {
      ...
      cite += (cite ? ", " : "") + "Vol " + ent.volume + issue;
    }
    if ("pages" in ent) {
      cite += (cite ? ", " : "") + "pp. " + ent.pages;
    }
    if (cite != "") cite += ". ";
    if ("publisher" in ent) {
      cite += ent.publisher;
  ...
  function title_string(ent) {
    return '<span class="title">' + texify(ent.title) + "</span> ";
  }
  ...
  function hover_cite(ent) {
    if (ent) {
      var cite = "";
      cite += "<strong>" + texify(ent.title) + "</strong>";
      cite += link_string(ent);
      cite += "<br>";
